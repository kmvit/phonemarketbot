...
def get_available_subcategories(parent_category, possible_subcats):
    """Получает список подкатегорий, которые есть в БД для родительской категории"""
...
def create_order(user_id, user_username, user_first_name, user_last_name):
    ...
    with get_db() as conn:
        cur = conn.cursor()
        # Получаем корзину в том же соединении
        cur.execute("""
            SELECT c.id, c.product_id, c.quantity, p.name, p.memory, p.color, p.country, p.price
            FROM cart c
            JOIN products p ON c.product_id = p.id
        ...
        for item in cart_items:
            ...
            cur.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (order_id, item['product_id'], item['name'], item['quantity'], final_price))
        
        # Очищаем корзину в том же соединении
        cur.execute("""
            DELETE FROM cart
            WHERE user_id=?
        """, (user_id,))
